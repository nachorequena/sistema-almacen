...
const EditCliente = () => {
  ...
  const obtenerCliente = async () => {
    ...
    if (snap.exists()) {
      ...
      setTipoCC(data.tipoCuentaCorriente);
    }
  };
  ...
  // Validar que no exista otro cliente con mismo nombre o teléfono
  const validarDuplicado = async () => {
    ...
    if (!snap1.empty) {
      const existeOtro = snap1.docs.some((d) => d.id !== id);
      if (existeOtro) return true;
    }
    ...
    if (telefono.trim() !== "") {
      ...
      if (!snap2.empty) {
        const existeOtro = snap2.docs.some((d) => d.id !== id);
        if (existeOtro) return true;
      }
    }
  ...
  const update = async (e) => {
    ...
    const existe = await validarDuplicado();
    if (existe) {
      Swal.fire(
        "Error",
